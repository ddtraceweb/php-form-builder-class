...
class Checksort extends Sort
{
    ...
    public function render()
    {
        ...
        foreach ($this->options as $value => $text) {
            ...
            echo '<label class="', $labelClass, '"><input id="', $this->_attributes["id"], '-', $count, '"', $this->getAttributes(
                array("id", "value", "checked", "name", "onclick", "required")
            ), ' value="', $this->filter($value), '"';
            if (in_array($value, $this->_attributes["value"])) {
                echo ' checked="checked"';
            }
            echo ' onclick="updateChecksort(this, \'', str_replace(
                array('"', "'"),
                array('&quot;', "\'"),
                $text
            ), '\');"/>', $text, '</label>';
